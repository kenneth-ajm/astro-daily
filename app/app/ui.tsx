  ...
  const generateReading = async () => {
    ...
    try {
      ...
      if (result.reading_json) {
        ...
        setReadingJson(null);
        setReadingText(result.reading as string);
      ...
      const msg = e instanceof Error ? e.message : "Unexpected error generating reading.";
      setStatus(msg);
    } finally {
      setGenerating(false);
    }
  ...
  return (
    <section className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Your Astrology Dashboard</h1>
          <p className="text-sm text-slate-400">Midnight Observatory edition ✨</p>
        </div>
        <button className="rounded border border-slate-700 px-3 py-2 text-sm hover:bg-slate-800" onClick={logout}>
          Sign out
        </button>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {mood ? (
            <div className="flex items-center gap-3 rounded-lg border border-slate-800 bg-slate-950 px-3 py-2">
              ...
              <div className="w-40 h-2 rounded bg-slate-800 overflow-hidden">
                <div className="h-2 bg-violet-500" style={{ width: `${mood.score}%` }} />
              </div>
              <div className="text-sm font-medium">{mood.score}</div>
        ...
        {readingJson ? (
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
              <h2 className="text-xl font-semibold">{readingJson.headline}</h2>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="text-xs uppercase tracking-wide text-emerald-400">Do</div>
                <p className="mt-2 text-slate-200">{readingJson.do}</p>
              </div>
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="text-xs uppercase tracking-wide text-rose-400">Avoid</div>
            ...
            {/* Lucky */}
            <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
              <div className="text-xs uppercase tracking-wide text-slate-400">Lucky</div>
              <div className="mt-3 grid gap-3 md:grid-cols-3">
                <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
                  <div className="text-xs text-slate-400">Color</div>
                  <div className="mt-1 text-lg font-semibold">{readingJson.lucky?.color}</div>
                </div>
                <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
                  <div className="text-xs text-slate-400">Number</div>
                  <div className="mt-1 text-lg font-semibold">{readingJson.lucky?.number}</div>
                </div>
                <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
                  <div className="text-xs text-slate-400">Time window</div>
                  <div className="mt-1 text-lg font-semibold">{readingJson.lucky?.time_window}</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="text-xs uppercase tracking-wide text-slate-400">Your blueprint</div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="text-xs uppercase tracking-wide text-slate-400">Chinese zodiac</div>
                <div className="mt-2 text-lg font-semibold">{readingJson.chinese_zodiac?.animal}</div>
                <ul className="mt-3 space-y-2 text-slate-200">
                  {(readingJson.chinese_zodiac?.traits ?? []).slice(0, 5).map((t, i) => (
                    <li key={i} className="flex gap-2">
                      <span className="text-slate-500">•</span>
                      <span>{t}</span>
                    </li>
                ...
                {readingJson.chinese_zodiac?.today_tip ? (
                  <div className="mt-4 rounded-xl border border-slate-800 bg-slate-900 p-4">
                    ...
                    <div className="mt-1 text-slate-200">{readingJson.chinese_zodiac.today_tip}</div>
                  </div>
                ) : null}
              </div>
            </div>
            ...
            {/* Transits */}
            <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
              <div className="text-xs uppercase tracking-wide text-slate-400">Today’s sky</div>
              <ul className="mt-3 space-y-2 text-slate-200">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="text-xs uppercase tracking-wide text-slate-400">Reflection</div>
                <p className="mt-2 text-slate-200">{readingJson.reflection_question}</p>
              </div>
              <div className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="text-xs uppercase tracking-wide text-slate-400">Affirmation</div>
          ...
          // OLD fallback
          <article className="rounded-xl border border-slate-800 bg-slate-950 p-4 whitespace-pre-line text-slate-100">
            {readingText}
          </article>
        ) : (
